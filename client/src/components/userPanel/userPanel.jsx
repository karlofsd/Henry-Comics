...
const User = () => {
    ...
    const handleChange = (e) => {
        if(e.target.name === 'firstname' || e.target.name === 'lastname'){
            setInfo({
            ...info,
            [e.target.name]:e.target.value.substring(0,1).toUpperCase().concat(e.target.value.substring(1))
        })}
        else{setInfo({
            ...info,
          ...
          <TabPane tabId="1"> 
            ...
            <div className="shadow profile">
                <div className="header">
                    {!activeForm ? (info.firstname !== null ? <h2>{`${fullname}`}</h2> : <h2>"Ingresá tu nombre"</h2>) :            
                    <Fragment>
                        <div>
                            <label>
                                Nombre:
                    ...
                    <form className='form-user' onSubmit={handleSubmit}>
                        ...
                        <div className='button-profile'>
                            {!activeForm ? <button className='btn btn-danger' type='submit' onClick={handleClick}>editar</button> :
                                <Fragment>
                                    <button className='btn btn-dark' type='submit' name='guardar' onClick={handleClick}>guardar</button>
                                    <button className='btn btn-dark' type='submit' name='cancelar' onClick={handleClick}>cancelar</button>
                                </Fragment>
                            }
                        </div>
                    </form>
          ...
          <TabPane tabId="2">
            <div>
                <div className='gral-order'>
                    ...
                    { 
                    order && <Orden order={order} setStatusG={setStatusG} statusG={statusG} getOrder={getOrder} user={user.id}/>              
                    }
                    </div>
                </div>
            </div>
